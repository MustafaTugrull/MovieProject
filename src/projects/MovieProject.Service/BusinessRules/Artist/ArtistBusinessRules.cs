...
namespace MovieProject.Service.BusinessRules.Artist;
...
public sealed class ArtistBusinessRules(IArtistRepository artistRepository)
{
    ...
    {
        var isPresent = await artistRepository.AnyAsync(x => x.Name == firstName && x.Surname == lastName);
        if (isPresent)
        {
    ...
    {
        var isPresent = await artistRepository.AnyAsync(x => x.Id == id);
        if (!isPresent)
        {
