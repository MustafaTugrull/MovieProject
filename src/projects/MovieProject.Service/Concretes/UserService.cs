...
public sealed class UserService(IUserRepository userRepository, UserBusinessRules businessRules, IMapper mapper) : IUserService
{
    ...
    public async Task<UserResponseDto> DeleteAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        UserResponseDto userResponse = mapper.Map<UserResponseDto>(deleted);
